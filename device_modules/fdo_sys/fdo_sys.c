	    ...
	    bool *has_more, bool *is_more, size_t mtu)
{
	...
	case FDO_SI_HAS_MORE_DSI:
		result = fdo_si_has_more_dsi(has_more);
		goto end;
	...
	case FDO_SI_GET_DSI_COUNT:
		...
		goto end;
	case FDO_SI_GET_DSI:
		result = fdo_si_get_dsi(mtu, module_message, module_val,
					module_val_sz, file_remaining, bin_len,
					bin_data, temp_module_val_sz);
		goto end;
	case FDO_SI_SET_OSI:
		result = fdo_si_set_osi(
		    module_message, module_val, module_val_sz, &strcmp_filedesc,
		    &strcmp_write, &strcmp_exec, &strcmp_execcb,
		    &strcmp_statuscb, &strcmp_fetch);
		...
		if (strcmp_filedesc == 0) {
			...
			result = fdo_si_set_osi_exec(
			    ...
			    &strcmp_exec, &strcmp_execcb);
			goto end;
		} else if (strcmp_statuscb == 0) {
			result =
			    fdo_si_set_osi_status_cb(&status_cb_array_length);
			goto end;
		} else if (strcmp_fetch == 0) {
			result = fdo_si_set_osi_fetch(bin_len);
			goto end;
		}
	default:
		result = FDO_SI_FAILURE;
...
static bool write_status_cb(char *module_message)
{
	...
	if (!module_message) {
		LOG(LOG_ERROR, "Module fdo_sys - Invalid params for "
	...
	if (memcpy_s(module_message, sizeof(message), message,
		     sizeof(message)) != 0) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to copy module message "
			       "status_cb\n");
		return false;
	}

	if (!fdow_start_array(fdow, 3)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to start inner "
			       "fdo_sys:status_cb array\n");
		return false;
	}

	if (!fdow_boolean(fdow, status_cb_iscomplete)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to write "
	...
	if (!fdow_end_array(fdow)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to end inner "
			       "fdo_sys:status_cb array\n");
		return false;
	}
...
static bool write_eot(char *module_message, int status)
{
	...
	if (!fdow_start_array(fdow, 1)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to start inner array "
			       "in fdo_sys:eot\n");
		return false;
	}
	...
	if (!fdow_end_array(fdow)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to end inner array in "
			       "fdo_sys:eot\n");
		return false;
	}
...
int fdo_si_failure(void)
{
	...
	if (!process_data(FDO_SYS_MOD_MSG_EXIT, NULL, 0, NULL, NULL, NULL, NULL,
			  NULL)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to perform "
			       "clean-up operations\n");
		   ...
		   uint8_t *bin_data, size_t temp_module_val_sz)
{
	...
	if (!fdow_encoder_init(fdow)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to initialize "
			       "FDOW encoder\n");
		goto end;
	}

	if (!hasmore || write_type == FDO_SYS_MOD_MSG_NONE) {
		LOG(LOG_ERROR, "Module fdo_sys - Invalid state\n");
		goto end;
	}
	...
	if (write_type == FDO_SYS_MOD_MSG_STATUS_CB) {
		...
		if (file_sz == 0 || file_sz != get_file_sz(filename) ||
			...
			if (!write_eot(module_message, fetch_data_status)) {
				LOG(LOG_ERROR, "Module fdo_sys - Failed to "
		...
		// error log and fail
		LOG(LOG_ERROR, "Module fdo_sys - Invalid module write state\n");
		goto end;
	}
	...
	*module_val_sz = temp_module_val_sz;
	if (memcpy_s(module_val, *module_val_sz, fdow->b.block,
		     *module_val_sz) != 0) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to copy "
		   ...
		   int *strcmp_statuscb, int *strcmp_fetch)
{
	...
	if (*strcmp_filedesc != 0 && *strcmp_exec != 0 && *strcmp_write != 0 &&
	    ...
	    *strcmp_fetch != 0) {
		LOG(LOG_ERROR, "Module fdo_sys - Invalid moduleMessage\n");
		result = FDO_SI_CONTENT_ERROR;
		goto end;
	}
	...
	if (0 != memcpy_s(fdor->b.block, *module_val_sz, module_val,
			  *module_val_sz)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to copy buffer "
			       "into temporary FDOR\n");
		goto end;
	}
	...
	if (!fdor_parser_init(fdor)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to init FDOR parser\n");
		goto end;
	}
	result = FDO_SI_SUCCESS;
end:
...
int fdo_si_set_osi_strcmp(size_t bin_len, uint8_t *bin_data)
{
	...
	if (!fdor_string_length(fdor, &bin_len)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to read "
			       "fdo_sys:filedesc length\n");
		goto end;
	}

	if (bin_len == 0) {
		LOG(LOG_ERROR, "Module fdo_sys - Empty value received for "
			       "fdo_sys:filedesc\n");
	...
	if (memset_s(filename, sizeof(filename), 0) != 0) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to clear "
...
int fdo_si_set_osi_write(size_t bin_len, uint8_t *bin_data)
{
	...
	if (!fdor_byte_string(fdor, bin_data, bin_len)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to read value for "
			       "fdo_sys:write\n");
		goto end;
	}
	...
	if (!process_data(FDO_SYS_MOD_MSG_WRITE, bin_data, bin_len, filename,
			  NULL, NULL, NULL, NULL)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to process value "
			       "for fdo_sys:write\n");
		goto end;
	}
	result = FDO_SI_SUCCESS;
			...
			int *strcmp_execcb)
{
	...
	if (exec_array_length == 0) {
		LOG(LOG_ERROR, "Module fdo_sys - Empty array received for "
	...
	if (!fdor_start_array(fdor)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to start "
	...
	if (!exec_instr) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to alloc for "
	...
	for (exec_array_index = 0; exec_array_index < (int)exec_array_length;
		...
		if (!exec_instr[exec_array_index]) {
			LOG(LOG_ERROR, "Module fdo_sys - Failed to alloc "
		...
		if (exec_array_index == ((int)exec_array_length - 1)) {
			if (memset_s(filename, sizeof(filename), 0) != 0) {
				LOG(LOG_ERROR, "Module fdo_sys - Failed "
	...
	result = fdo_end(result, NULL, exec_instr);
	return result;
}
...
int fdo_si_set_osi_status_cb(size_t *status_cb_array_length)
{
	...
	if (!fdor_array_length(fdor, status_cb_array_length)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to process "
			       "fdo_sys:status_cb array length\n");
		goto end;
	}
	if (*status_cb_array_length != 3) {
		LOG(LOG_ERROR, "Module fdo_sys - Invalid number of items "
	...
	if (!fdor_end_array(fdor)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to end "
			       "fdo_sys:status_cb array\n");
		goto end;
	}
	...
	if (!process_data(FDO_SYS_MOD_MSG_STATUS_CB, NULL, 0, NULL, NULL,
			  &status_cb_iscomplete, &status_cb_resultcode,
			  &status_cb_waitsec)) {
		LOG(LOG_ERROR, "Module fdo_sys - Failed to "
